...
const Category = () => {
    ...
    function handleInput(type: "title" | "text" | "seodescription" | "seotitle", value: string) {
        if (categoryData !== null) {
            ...
            if (type === "seodescription") changeCategoryData({ id: categoryData?.id, data: { ...categoryData?.data, seo: { ...categoryData.data.seo, description: value } } })
            if (type === "seotitle") changeCategoryData({ id: categoryData?.id, data: { ...categoryData?.data, seo: { ...categoryData.data.seo, title: value } } })
        }
    }

    function setCategory(value: string) {
        if (categoryData !== null) {
            changeCategoryData({ id: categoryData?.id, data: { ...categoryData?.data, category: value } })
        }
    }
    ...
    const loadImage = useCallback(async (path: string) => {
        if (path !== "") {
            ...
            try {
                ...
                if (categoryData !== null) {
                    changeCategoryData({ id: categoryData?.id, data: { ...categoryData?.data, image: url } })
                }
            } catch (error) {
    ...
    return (
        <div className={styles.Box} onClick={e => e.stopPropagation()}>
            ...
            <div className={styles.List}>
                ...
                <Select array={catalog} value={categoryData?.data.category || ""} setValue={setCategory} />
                ...
                <h3>SEO</h3>
                <Input label="Заголовок" onChange={e => handleInput("seotitle", e.target.value)} type="text" value={categoryData?.data.seo.title || ""} />
                <Textarea label="Описание" onChange={e => handleInput("seodescription", e.target.value)} value={categoryData?.data.seo.description || ""} />
            </div>
